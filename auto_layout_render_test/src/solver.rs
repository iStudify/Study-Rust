...
use crate::layout::*;
use cassowary::{
    Solver, Variable, Constraint as CassowaryConstraint, 
    WeightedRelation::*, AddConstraintError, SuggestValueError, strength::*
};
...
impl LayoutSolver {
    ...
    fn setup_canvas_constraints(&mut self, canvas: &Canvas) -> Result<(), SolverError> {
        // 画布位置固定在原点
        self.solver.add_constraint(self.canvas_vars.x | EQ(REQUIRED) | 0.0)?;
        self.solver.add_constraint(self.canvas_vars.y | EQ(REQUIRED) | 0.0)?;
        
        // 画布尺寸
        self.solver.add_constraint(self.canvas_vars.width | EQ(REQUIRED) | canvas.width as f64)?;
        self.solver.add_constraint(self.canvas_vars.height | EQ(REQUIRED) | canvas.height as f64)?;
        
        // 计算画布的中心点和右下角
        self.solver.add_constraint(
            self.canvas_vars.center_x | EQ(REQUIRED) | (self.canvas_vars.x + self.canvas_vars.width / 2.0)
        )?;
        self.solver.add_constraint(
            self.canvas_vars.center_y | EQ(REQUIRED) | (self.canvas_vars.y + self.canvas_vars.height / 2.0)
        )?;
        self.solver.add_constraint(
    ...
    fn add_basic_constraints(&mut self) -> Result<(), SolverError> {
        ...
        for vars in self.variables.values() {
            // center_x = x + width / 2
            self.solver.add_constraint(
                vars.center_x | EQ(REQUIRED) | (vars.x + vars.width / 2.0)
            )?;
            
            // center_y = y + height / 2
            self.solver.add_constraint(
                vars.center_y | EQ(REQUIRED) | (vars.y + vars.height / 2.0)
            )?;
            
            // right = x + width
            self.solver.add_constraint(
                vars.right | EQ(REQUIRED) | (vars.x + vars.width)
            )?;
            
            // bottom = y + height
            self.solver.add_constraint(
                vars.bottom | EQ(REQUIRED) | (vars.y + vars.height)
            )?;
            
            // 尺寸必须为正数
            self.solver.add_constraint(vars.width | GE(REQUIRED) | 0.0)?;
            self.solver.add_constraint(vars.height | GE(REQUIRED) | 0.0)?;
        }
    ...
    fn add_user_constraints(&mut self, elements: &[Element]) -> Result<(), SolverError> {
        for element in elements {
            ...
            // 处理堆叠容器的特殊约束
            match element {
                Element::VStack { children, properties, .. } => {
                    self.add_vstack_constraints(element.id(), children, properties)?;
                }
                Element::HStack { children, properties, .. } => {
    ...
    /// 添加单个约束
    fn add_constraint(&mut self, element_id: &ElementId, constraint: &Constraint) -> Result<(), SolverError> {
        let vars = self.variables.get(element_id)
        ...
        match &constraint.constraint_type {
            ConstraintType::Top { target, value } => {
                ...
                self.solver.add_constraint(
                    vars.y | EQ(strength) | (target_var + *value as f64)
                )?;
            }
            ...
            ConstraintType::Bottom { target, value } => {
                ...
                self.solver.add_constraint(
                    vars.bottom | EQ(strength) | (target_var - *value as f64)
                )?;
            }
            ...
            ConstraintType::Leading { target, value } => {
                ...
                self.solver.add_constraint(
                    vars.x | EQ(strength) | (target_var + *value as f64)
                )?;
            }
            ...
            ConstraintType::Trailing { target, value } => {
                ...
                self.solver.add_constraint(
                    vars.right | EQ(strength) | (target_var - *value as f64)
                )?;
            }
            ...
            ConstraintType::CenterX { target, offset } => {
                ...
                self.solver.add_constraint(
                    vars.center_x | EQ(strength) | (target_var + *offset as f64)
                )?;
            }
            ...
            ConstraintType::CenterY { target, offset } => {
                ...
                self.solver.add_constraint(
                    vars.center_y | EQ(strength) | (target_var + *offset as f64)
                )?;
            }
            ...
            ConstraintType::Width { value, target, multiplier, percent } => {
                if let Some(width_value) = value {
                    self.solver.add_constraint(
                        vars.width | EQ(strength) | (*width_value as f64)
                    )?;
                    ...
                    if let Some(percent_value) = percent {
                        self.solver.add_constraint(
                            vars.width | EQ(strength) | (target_var * (*percent_value as f64 / 100.0))
                        )?;
                    } else {
                        self.solver.add_constraint(
            ...
            ConstraintType::Height { value, target, multiplier, percent } => {
                if let Some(height_value) = value {
                    self.solver.add_constraint(
                        vars.height | EQ(strength) | (*height_value as f64)
                    )?;
                } else if let Some(target_id) = target {
                    let target_var = if target_id == "canvas" {
                    ...
                    if let Some(percent_value) = percent {
                        self.solver.add_constraint(
                            vars.height | EQ(strength) | (target_var * (*percent_value as f64 / 100.0))
                        )?;
                    } else {
                        self.solver.add_constraint(
            ...
            ConstraintType::AspectRatio { ratio } => {
                self.solver.add_constraint(
                    vars.width | EQ(strength) | (vars.height * *ratio as f64)
                )?;
            }
            
            ConstraintType::MinWidth { value } => {
                self.solver.add_constraint(
                    vars.width | GE(strength) | (*value as f64)
                )?;
            }
            
            ConstraintType::MaxWidth { value } => {
                self.solver.add_constraint(
                    vars.width | LE(strength) | (*value as f64)
                )?;
            }
            
            ConstraintType::MinHeight { value } => {
                self.solver.add_constraint(
                    vars.height | GE(strength) | (*value as f64)
                )?;
            }
            
            ConstraintType::MaxHeight { value } => {
                self.solver.add_constraint(
                    vars.height | LE(strength) | (*value as f64)
                )?;
            }
            ...
            _ => {
                ...
                return Err(SolverError::InvalidConstraint(
                    format!("Unsupported constraint type: {:?}", constraint.constraint_type)
                ));
            }
        }
    ...
    fn add_vstack_constraints(
        ...
        let stack_vars = self.variables.get(stack_id)
            .ok_or_else(|| SolverError::ElementNotFound(stack_id.clone()))?;
        
        // 垂直排列：每个子元素的顶部等于前一个元素的底部加间距
        for (i, child) in children.iter().enumerate() {
            let child_vars = self.variables.get(child.id())
                .ok_or_else(|| SolverError::ElementNotFound(child.id().clone()))?;
            
            if i == 0 {
                // 第一个元素顶部对齐到容器顶部
                self.solver.add_constraint(
                    child_vars.y | EQ(REQUIRED) | stack_vars.y
                )?;
            } else {
                // 其他元素顶部等于前一个元素底部加间距
                let prev_child = &children[i - 1];
                let prev_vars = self.variables.get(prev_child.id())
            ...
            match properties.alignment {
                Alignment::Leading => {
                    self.solver.add_constraint(
                        child_vars.x | EQ(REQUIRED) | stack_vars.x
                    )?;
                }
                Alignment::Center => {
                    self.solver.add_constraint(
                        child_vars.center_x | EQ(REQUIRED) | stack_vars.center_x
                    )?;
                }
                Alignment::Trailing => {
                    self.solver.add_constraint(
                        child_vars.right | EQ(REQUIRED) | stack_vars.right
                    )?;
                }
                _ => {}
            }
        ...
        if let Some(last_child) = children.last() {
            let last_vars = self.variables.get(last_child.id())
    ...
    fn add_hstack_constraints(
        ...
        for (i, child) in children.iter().enumerate() {
            ...
            match properties.alignment {
                ...
                Alignment::Center => {
                    self.solver.add_constraint(
                        child_vars.center_y | EQ(REQUIRED) | stack_vars.center_y
                    )?;
                }
                Alignment::Bottom => {
                    self.solver.add_constraint(
                        child_vars.bottom | EQ(REQUIRED) | stack_vars.bottom
                    )?;
                }
                _ => {}
            }
