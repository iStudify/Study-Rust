            ...
            return Err(RenderError::ImageError(image::ImageError::Unsupported(
                image::error::UnsupportedError::from_format_and_kind(
                    ...
                    image::error::UnsupportedErrorKind::GenericFeature(
                        "Network images not supported".to_string(),
                    ),
                ),
            )));
        } else {
...
impl Renderer {
    ...
    fn render_element(
        ...
        let frame = computed_layout
            .get_frame(element.id())
    ...
    fn render_text(
        ...
        let font = self
            .context
            .fonts
            .get(&properties.font_family)
            .ok_or_else(|| {
                RenderError::FontError(format!("Font not loaded: {}", properties.font_family))
            })?;
        ...
        // 使用 imageproc 渲染文本
        draw_text_mut(image, color, x as i32, y as i32, scale, font, content);
    ...
    fn render_image(
        ...
        let src_image = self.context.images.get(source).ok_or_else(|| {
            RenderError::ImageError(image::ImageError::Unsupported(
                image::error::UnsupportedError::from_format_and_kind(
                    image::error::ImageFormatHint::Unknown,
    ...
    fn calculate_scaled_size(
        ...
        match scale_mode {
            ScaleMode::Fit => {
                ...
                (
                    (src_width as f32 * scale) as u32,
                    (src_height as f32 * scale) as u32,
                )
            }
            ScaleMode::Fill => {
                let scale_x = dst_width as f32 / src_width as f32;
            ...
            ScaleMode::Stretch => (dst_width, dst_height),
            ScaleMode::Center => (src_width, src_height),
        }
    }
}
...
fn alpha_blend(background: Rgba<u8>, foreground: Rgba<u8>) -> Rgba<u8> {
    ...
    let r =
        ((fg[0] as f32 * alpha_fg + bg[0] as f32 * alpha_bg * (1.0 - alpha_fg)) / alpha_out) as u8;
    let g =
        ((fg[1] as f32 * alpha_fg + bg[1] as f32 * alpha_bg * (1.0 - alpha_fg)) / alpha_out) as u8;
    let b =
